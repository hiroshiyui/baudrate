...
|-------------|---------|---------|
| Elixir | the version in `.tool-versions` (1.20.4-otp-29) | Application runtime |
...
1. Take the public key **from the profile page**, not from any mail, and import
   ...
   truth.
2. Click **Issue challenge** on that contact. Baudrate writes a one-line
   phrase naming the instance, the account and the date, with a random nonce
   (ADR 0067). Send that exact line to the registered address and ask for it
   back, signed. **Mail to send** under the challenge holds a subject and a
   message ready to paste, written in the member's own language, with the
   phrase already in it — edit it freely, but leave that line alone.
3. Check that the signature verifies **against the key from the profile page**,
   that it covers the challenge you issued, and that the From address is one
   they registered.
4. Only then click **Mark verified**. That spends the challenge: it works
...
4. **Send a challenge.** On `/admin/users/:id`, click **Issue challenge** on
   that contact and send the line it shows — **Mail to send** has a subject
   and a message ready to paste. Ask for it back, signed.
   ...
   is the single strongest check in the procedure, which is why the instance
   now issues the phrase and **Issue reset link** refuses without one
   (ADR 0067) — it used to be a nonce you generated by hand, and a step done
   by hand is a step that gets skipped. You no longer need `openssl rand`.

   Baudrate still checks no signature. It asks the question; your client
   answers it.
...
(e.g. `"1.40.1"` for tag `v1.40.1`). This version appears in the release
