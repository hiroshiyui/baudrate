...
Directories only; each module's `@moduledoc` says what it does (`mix docs`, or read
the file). A per-file listing lived here until 2026-09 and went stale faster than it
was read.

```
native/                    # Rust NIFs (Rustler)
├── baudrate_sanitizer/    #   Ammonia: sanitize_federation, sanitize_markdown, strip_tags
├── baudrate_html_parser/  #   scraper/html5ever: Open Graph, URL extraction, image counts
└── baudrate_feed_parser/  #   feedparser-rs: RSS / Atom / JSON Feed
lib/
├── baudrate/              # Contexts: one facade module per context (auth.ex, content.ex,
│   │                      #   federation.ex, …) over a directory of focused sub-modules
│   ├── auth/              #   users, passwords, sessions, 2FA, WebAuthn, recovery, sanctions,
│   │                      #   blocks and mutes, invites, IP bans, proof of work, trust
│   ├── content/           #   boards, articles, comments, likes, polls, tags, search,
│   │                      #   drafts, watches, permissions, sitemap, feeds
│   ├── federation/        #   actors, inbox/outbox, delivery and circuits, HTTP client,
│   │                      #   signatures, validator, resolvers, collections, timeline
│   ├── messaging/, moderation/, notification/, bots/, media/, crypto/, backup/,
│   ├── health/, setup/, data_portability/, account_deletion/, account_migration/,
│   └── announcements/, sanitizer/, html_parser/, logger/
│      (release.ex: release tasks; retention.ex, dashboard.ex, pagination.ex, repo.ex)
├── baudrate_web/          # Web layer
│   ├── components/        #   layouts, core and comment components
│   ├── controllers/       #   ActivityPub, feeds, sessions, media, pages
│   ├── live/              #   LiveViews (`*_live.ex` + `.html.heex`), auth hooks; admin/
│   ├── plugs/             #   RealIp, EnsureSetup, HTTP signatures, rate limits, locale
│   └── (router, endpoint, helpers, safe_html, locale, rate limiter, crawlers, …)
└── mix/tasks/             # backup, restore, backfills, selenium_setup
assets/js/                 # app.js and the LiveView hooks (see LiveView JS Hooks)
priv/gettext/              # en, zh_TW, ja_JP
test/                      # mirrors lib/; test/doc gates the docs, test/ops the nginx config
ci/, ansible/, scripts/    # CI image, deployment, operator scripts
```
