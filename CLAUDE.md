...
- Only boards with `min_role_to_view == "guest"` and `ap_enabled == true` are federated. A non-federated board's inbox/outbox/actor endpoints return 404, and Follow activities targeting a non-federated board actor (via the shared inbox) are answered with `Reject(Follow)`. **Exception:** remote articles (`remote_actor_id` non-nil) that happen to reside in a non-federated board still accept Like / Announce (boost) / Create (reply) activities — they already exist on the fediverse with their own `ap_id`, so refusing interactions there would break federation semantics. Local articles in non-federated boards remain non-federated and reject incoming activities.
