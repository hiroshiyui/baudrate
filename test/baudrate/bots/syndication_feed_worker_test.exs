defmodule Baudrate.Bots.FeedWorkerTest do
  @moduledoc """
  Pure-function coverage for `Baudrate.Bots.SyndicationFeedWorker`.
  ...
  describe "build_slug/2" do
    ...
    test "truncates the title portion to 60 characters before appending the hash" do
      ...
      slug = SyndicationFeedWorker.build_slug(long_title, "g")
    ...
    test "deterministic — same title + guid yields the same slug" do
      assert SyndicationFeedWorker.build_slug("Title", "abc") ==
               SyndicationFeedWorker.build_slug("Title", "abc")
    end

    test "different guids yield different slugs even for the same title" do
      a = SyndicationFeedWorker.build_slug("Title", "guid-a")
      b = SyndicationFeedWorker.build_slug("Title", "guid-b")
      refute a == b
    ...
    test "falls back to the bare hash when the title contains no slug-safe characters" do
      slug = SyndicationFeedWorker.build_slug("！？@#$", "guid-fallback")
      assert slug =~ ~r/\A[a-f0-9]{8}\z/
    end

    test "falls back to the bare hash for an empty title" do
      slug = SyndicationFeedWorker.build_slug("", "guid-empty")
      assert slug =~ ~r/\A[a-f0-9]{8}\z/
    end
    ...
    test "non-ASCII titles fall through to the hash since they contain no [a-z0-9]" do
      ...
      slug_ja = SyndicationFeedWorker.build_slug("こんにちは", "guid-ja")
      slug_zh = SyndicationFeedWorker.build_slug("你好", "guid-zh")
    ...
    test "matches the slug format expected by Article.changeset" do
      slug = SyndicationFeedWorker.build_slug("Whatever", "guid-format")
