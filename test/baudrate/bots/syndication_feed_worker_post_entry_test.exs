defmodule Baudrate.Bots.FeedWorkerPostEntryTest do
  @moduledoc """
  Integration coverage for `SyndicationFeedWorker.post_entry/2`'s failure handling.
  ...
  test "records the syndication entry without crashing when the article slug collides",
       %{bot: bot, board: board} do
    title = "Colliding Title"
    guid = "guid-collision-#{System.unique_integer([:positive])}"
    slug = SyndicationFeedWorker.build_slug(title, guid)
    ...
    SyndicationFeedWorker.post_entry(bot, entry)
