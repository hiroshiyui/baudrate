defmodule Baudrate.Content.Pagination do
  @moduledoc """
  Shared pagination helpers for paginated content queries.

  Extracts the repeated count-paginate-wrap pattern from `Content`
  ...
  ## Examples
      ...
      iex> paginate_opts([page: 3, per_page: 10], 20)
      {3, 10, 20}

      iex> paginate_opts([], 20)
      {1, 20, 0}

      iex> paginate_opts([page: -1], 20)
      {1, 20, 0}
  """
  @spec paginate_opts(keyword(), pos_integer()) ::
          {pos_integer(), pos_integer(), non_neg_integer()}
  def paginate_opts(opts, default_per_page) do
