...
0. **Check the branch** — all development is committed on `current`. If `git branch --show-current` is `main`, run `git switch current` first (uncommitted changes carry over). Never commit or push to `main`; it only moves during release engineering.
